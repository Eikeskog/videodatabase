...
def gmaps_geocode_get_address_dict(address_components_json):
  ...
  def get_component_type(component_json):
    return str(
      max([type for type in component_json['types'] if type != 'political'], key=len) if len(component_json['types']) > 0
  ...
  parsed_dict = add_missing_dict_keys({
    get_column_name(component) :
    get_component_value(component)
    for component in address_components_json
  })

  parsed_dict['country_code'] = [component["short_name"] for component in address_components_json if "country" in component["types"]][0]
...
def gmaps_geocode_get_displaynames(address_dict):
  if not address_dict or not isinstance(address_dict, dict):
    return None
  exclude_columns = ['postal_code', 'street_number', 'displayname_short', 'displayname_full', 'formatted_address', 'country']

  if address_dict['county'] == 'Oslo': 
    exclude_columns += ['county', 'municipality']

  def get_displayname_full(address_dict):
    displayname_full = ",".join(list(dict.fromkeys([
      column for column, value in address_dict.items()
    ...
    pos2 = [
      x for x in [
        ...
        'country_code'
          ...
          address_dict[x] is None,
          x == pos1[0]
        ])
      ] or [None]

    displayname_short = ",".join(list(dict.fromkeys(
      [x for x in [pos1[0], pos2[0]] if x is not None]))) if any(
        [pos1[0], pos2]) else None
...
def gmaps_geocode_get_boundingbox(bounds_json):
  ...
  bounds_lat = [bounds_json['northeast']['lat'], bounds_json['southwest']['lat'],]
  bounds_lng = [bounds_json['northeast']['lng'], bounds_json['southwest']['lng'],]

  point_nw   = (bounds_json['northeast']['lat'], bounds_json['southwest']['lng'],)
  point_ne   = (bounds_json['northeast']['lat'], bounds_json['northeast']['lng'],)
  point_sw   = (bounds_json['southwest']['lat'], bounds_json['southwest']['lng'],)
...
def get_reverse_geotags_gmaps(latlng, levels=5): 
  ...
  json = [x for x in gmaps.reverse_geocode(latlng)
    if not any([
      x in null,
  ...
  for res in json:
    ...
    if not any([
      ...
      if "viewport" in res["geometry"] and isinstance(res["geometry"]["viewport"], dict):
