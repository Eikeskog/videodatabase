...
countries_zip_path = sys.path[0] + '\\countries.zip' #videodb\\utils\\classes\\countries\\countries.zip' 
...
class Countries:
  ...
  def get_translation(cls, country_code, lang_code):
    if not country_code in cls._countries:
      raise NotSupportedError(country_code=country_code)
    if not lang_code in cls._languages:
...
class Country:
  def __init__(self, country_code, language_code='en'):
    ...
    self._translated_name = Countries.get_translation(self._country_code,self._language_code)
  ...
  def translate_to(cls, self, language_code):
    self._language_code = language_code.lower()
    self._translated_name = cls.get_translation(self._country_code,self._language_code)
    return self._translated_name
