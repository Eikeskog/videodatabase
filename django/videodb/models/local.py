...
class RootDirectory(models.Model):
    class Meta: db_table = 'root_directory'
    ...
    def path_as_instance(cls, path: str) -> object:
        ...
        if obj.disk is None:
            ...
            if disk_serial_number:
                disk_obj = Disk.objects.filter(disk_serial_number = disk_serial_number).first()
                if not disk_obj:
                    disk_obj = Disk.objects.create(disk_serial_number = disk_serial_number)
                obj.disk = disk_obj
                obj.save()
        return obj
...
class ProjectMainDirectory(models.Model):
    class Meta: db_table = 'project_main_directory'
    ...
    def path_as_instance(cls, path: str) -> object:
        ...
        if obj.project is None:
            ...
            if project_name:
                obj.project = Project.objects.get_or_create(
                    name = project_name
                )[0]
                obj.save()
...
class ProjectRollDirectory(models.Model):
    class Meta: db_table = 'project_roll_directory'
    ...
    def path_as_instance(cls, path: str) -> object:
        ...
        if obj.project_main_directory is None:
            ...
            if is_valid_db_path(main_dir_path):
                obj.project_main_directory = ProjectMainDirectory.path_as_instance(main_dir_path)
                obj.save()
        return obj
...
class LocalDirectory(models.Model):
    ...
    parent_foreign = models.ForeignKey(to='self', null=True, blank=True, on_delete=models.CASCADE)
    ...
    def path_as_instance(cls, path: str) -> object:
        ...
        if obj.project_roll_directory is None:
            ...
            if is_valid_db_path(roll_dir_path):
                obj.project_roll_directory = ProjectRollDirectory.path_as_instance(obj.path)
...
class LocalFile(models.Model):
    class Meta: db_table = 'local_file'

    file_path = models.CharField(max_length=255, primary_key=True)
    file_rolltype = models.CharField(max_length=255, null=True, blank=True) # denne kan være i directory
    ...
    videoitem = models.ForeignKey(to='Videoitem', on_delete=models.CASCADE, null=True, blank=True, related_name='local_paths')
    ...
    def get_directory(self) -> dict:
        ...
        return {
            ...
            'videoitems': directory.localfile_set.all().annotate(
