...
class Keyword(models.Model):
    ...
    unique_keyword = models.ForeignKey(
        ...
        related_name="keyword_videoitem_link",
    )
    ...
    def create_if_valid(cls, keyword, videoitem=None):
        ...
        if valid_string:
            cls.objects.get_or_create(keyword=valid_string, videoitem=videoitem)

    @classmethod
    def validate_and_clean_all(cls) -> None:
        qs = cls.objects.all()
        for obj in qs:
            ...
            if obj.unique_keyword is None:
                unique_keyword = UniqueKeyword.objects.get_or_create(
                    keyword=obj.keyword
                )
