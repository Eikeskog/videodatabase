...
class ScheduledReverseGeotag(models.Model):
    ...
    @classmethod
    def batch_reverse_geocode(cls) -> None:
        ...
        for scheduled in qs:
            ...
            if not geotag:
                time_start = timer()
                geotag_dict = cls.get_gmaps_geotag_dict((scheduled.lat, scheduled.lng,))
                if not geotag_dict:
                    continue
...
class GeotagLevel5(models.Model):
    ...
    def new_from_dict(cls, geotag_dict: dict) -> object:
        ...
        if not "level_5" in geotag_dict or not isinstance(geotag_dict["level_5"], dict):
            return None
...
class GeotagLevel4(models.Model):
    ...
    def new_from_dict(cls, geotag_dict: dict) -> object:
        if not geotag_dict or not isinstance(geotag_dict, dict):
            return None
        if not "level_4" in geotag_dict or not isinstance(geotag_dict["level_4"], dict):
            return None
        if not all([
...
class GeotagLevel3(models.Model):
    ...
    def new_from_dict(cls, geotag_dict: dict) -> object:
        ...
        if not "level_3" in geotag_dict or not isinstance(geotag_dict["level_3"], dict):
            return None
...
class GeotagLevel2(models.Model):
    ...
    def new_from_dict(cls, geotag_dict: dict) -> object:
        if not geotag_dict or not isinstance(geotag_dict, dict):
            return None
        if not "level_2" in geotag_dict or not isinstance(geotag_dict["level_2"], dict):
            return None
        if not all([
...
class GeotagLevel1(models.Model):
    ...
    def new_from_dict(cls, geotag_dict: dict) -> object:
        if not geotag_dict or not isinstance(geotag_dict, dict):
            return None
        if not "level_1" in geotag_dict or not isinstance(geotag_dict["level_1"], dict):
            return None
        if not all([
